export type SaveOnTheStripEventType =
  | "handoff_viewed"
  | "lead_captured"
  | "booking_started"
  | "booking_completed"
...
export type SaveOnTheStripPayload = {
  handoffId: string;
  satelliteId: "saveonthestrip";
  eventType: SaveOnTheStripEventType;
  occurredAt?: string;
  source?: string;
  sourcePath?: string;
...
export async function emitDccEvent(payload: SaveOnTheStripPayload) {
  const token = process.env.DCC_SAVEONTHESTRIP_WEBHOOK_TOKEN;
  if (!token) {
