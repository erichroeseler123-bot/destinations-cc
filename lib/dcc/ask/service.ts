...
const SYNONYMS: Record<string, string[]> = {
  denver: ["den", "colorado"],
  vail: ["mountain", "resort", "ski"],
  breckenridge: ["mountain", "resort", "ski"],
  juneau: ["alaska", "cruise", "helicopter", "glacier"],
  helicopter: ["flight", "flightseeing", "glacier", "juneau"],
  whale: ["whales", "watching", "juneau", "alaska"],
  swamp: ["airboat", "covered", "boat", "new", "orleans"],
  airboat: ["swamp", "boat", "new", "orleans"],
  bourbon: ["french", "quarter", "new", "orleans"],
  royal: ["french", "quarter", "new", "orleans"],
  thomas: ["st", "usvi", "cruise", "driver"],
  croix: ["st", "usvi", "cruise", "driver"],
  john: ["st", "usvi", "cruise", "driver"],
  driver: ["private", "local", "vibe", "cruise"],
  red: ["rocks", "concert", "transportation"],
  argo: ["idaho", "springs", "shuttle", "denver"],
  dells: ["wisconsin", "family", "group"],
};

export type AskDccSource = {
  slug: string;
  title: string;
  href: string;
  answer: string;
...
export type AskDccEvidence = {
  sources: AskDccSource[];
  handoff: AskDccHandoff | null;
...
function siteForHref(href: string) {
  return sitesRegistry.sites.find((site) => href.includes(site.domain));
}

function handoffForSite(siteId: string) {
  return handoffsRegistry.handoffs.find((handoff) => handoff.from === DCC_SITE_ID && handoff.to === siteId);
}
...
export function getAskDccEvidence(question: string): AskDccEvidence {
  ...
  const sources: AskDccSource[] = ranked.map(({ guide }) => ({
    slug: guide.slug,
    title: guide.title,
  ...
  const handoff = targetSite && governedHandoff
    ? {
        siteId: targetSite.site_id,
        siteName: targetSite.name,
        href: leadingGuide!.nextStep.href,
        reason: governedHandoff.when,
        terminal: governedHandoff.terminal,
      }
    : null;
...
export function fallbackAskDccAnswer(question: string, evidence: AskDccEvidence) {
  if (!evidence.sources.length) {
    return "I don’t have a strong enough DCC match for that question yet. Try adding the destination, timing, group size, or the choice you are weighing so I can route you to a real decision instead of guessing.";
  }
...
export function buildAskDccPrompt(input: {
  question: string;
  history: Array<{ role: "user" | "assistant"; content: string }>;
  evidence: AskDccEvidence;
}) {
  const evidenceText = input.evidence.sources.map((source, index) =>
    `[${index + 1}] ${source.title}\nURL: ${source.href}\nDCC answer: ${source.answer}`
  ).join("\n\n");

  const handoffText = input.evidence.handoff
    ? `Allowed next commercial handoff: ${input.evidence.handoff.siteName} (${input.evidence.handoff.href}). Reason: ${input.evidence.handoff.reason}.`
    : "No governed commercial handoff is currently justified.";
