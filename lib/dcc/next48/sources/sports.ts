...
export async function fetchSportsItems(
  ...
  if (context.debugFailSource === "sports") {
    throw new Error("debug failure for sports source");
  }

  const seed = getNext48CuratedSeed(context.entityType, context.slug);
  if (!seed) return { source: "sports", status: "ok", items: [] };

  const items: Next48CandidateItem[] = seed.items
    .filter((item) => item.source === "sports")
    .map((item) => ({
      id: item.id,
      source: "sports",
  ...
  return { source: "sports", status: "ok", items };
}
