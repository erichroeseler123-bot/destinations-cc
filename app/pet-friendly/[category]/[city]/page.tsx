...
export function generateStaticParams() {
  return listOverlayCitySlugsByType("pet-friendly").flatMap((city) =>
    listOverlayCategoriesByTypeAndCity("pet-friendly", city).map((category) => ({
...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  ...
  const overlay = getOverlayRegistryNodeByTypeAndCity("pet-friendly", city);
  const cityNode = getCityRegistryNode(city);
  if (!overlay || !cityNode) return {};
  return buildOverlayCategoryMetadata(cityNode, overlay, category, `/pet-friendly/${category}/${city}`);
}
