...
export function generateStaticParams() {
  return listRoadTripRelationshipSlugs("diners-near").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<Params> }) {
  const { slug } = await params;
  const page = getRoadTripRelationship("diners-near", slug);
  if (!page) return {};
  return buildRoadTripRelationshipMetadata(page);
}

export default async function DinersNearPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params;
  const page = getRoadTripRelationship("diners-near", slug);
  if (!page) notFound();
