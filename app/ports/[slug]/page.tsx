...
export default async function PortPage({ params }: { params: { slug: string } }) {
  ...
  if (!res.ok) {
    return <div className="p-8 text-red-600">Error loading data (status {res.status})</div>;
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        <p><strong>Location:</strong> {port.city}, {port.region}, {port.country}</p>
        <p><strong>Coordinates:</strong> {port.lat}, {port.lng}</p>
        <p><strong>Dock Notes:</strong> {port.dock_notes || 'N/A'}</p>
        <p><strong>Seasonal:</strong> {port.seasonal_notes || 'N/A'}</p>
        <p><strong>Passengers:</strong> {port.passenger_volume?.toLocaleString() || 'N/A'} ({port.volume_year})</p>
