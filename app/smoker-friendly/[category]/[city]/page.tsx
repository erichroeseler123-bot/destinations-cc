...
export function generateStaticParams() {
  return listOverlayCitySlugsByType("smoker-friendly").flatMap((city) =>
    listOverlayCategoriesByTypeAndCity("smoker-friendly", city).map((category) => ({
...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  ...
  const overlay = getOverlayRegistryNodeByTypeAndCity("smoker-friendly", city);
  const cityNode = getCityRegistryNode(city);
  if (!overlay || !cityNode) return {};
  return buildOverlayCategoryMetadata(cityNode, overlay, category, `/smoker-friendly/${category}/${city}`);
}
