...
export default function CityNetworkHandoff({ citySlug, cityName }: { citySlug: string; cityName: string }) {
  const routes = getCityNetworkRoutes(citySlug);
    ...
    <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm leading-6 text-white/55">These are contextual handoffs into the wider destination network. DCC stays the live research layer; the linked site takes over only when it has a more specific job to do.</p>
        </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {routes.slice(0, 6).map((route) => (
          <a key={route.id} href={route.href} className="group rounded-[20px] border border-white/10 bg-black/20 p-4 transition hover:-translate-y-0.5 hover:border-cyan-300/25 hover:bg-white/[0.06]">
