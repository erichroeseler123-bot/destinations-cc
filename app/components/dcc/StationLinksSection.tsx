...
import { getStationSubtypeLabel, type StationView } from "@/lib/dcc/stations";

export default function StationLinksSection({
  title,
  intro,
  stations,
}: {
  ...
  return (
    <section className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(8,13,26,0.96),rgba(6,9,18,0.96))] p-6 sm:p-8">
      <div className="text-[11px] font-black uppercase tracking-[0.22em] text-[#8fd0ff]">Nearby transit</div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {stations.map((station) => (
          ...
          >
            ...
            <p className="mt-3 text-sm leading-6 text-white/70">{station.transferFocus}</p>
            ...
            <div className="mt-5 text-sm font-bold text-[#3df3ff]">Open station guide →</div>
          </Link>
        ))}
      </div>
    </section>
