...
import { beautifyCategory, getAnchorLabel, titleize } from "@/lib/dcc/liveCity/presentation";
    ...
    <article className="rounded-[1.6rem] border border-white/10 bg-black/25 p-5 shadow-[0_18px_50px_rgba(0,0,0,0.28)]">
      ...
      {(signal.linked_venue_slug || signal.linked_place_slug) ? (
        <div className="mt-4 text-xs uppercase tracking-[0.18em] text-zinc-500">
          {signal.linked_venue_slug ? `Venue: ${beautifyCategory(signal.linked_venue_slug)}` : `Place: ${beautifyCategory(signal.linked_place_slug ?? "")}`}
        </div>
      ) : null}
    </article>
  );
...
export function buildLiveCityNowMetadata(city: LiveCityKey): Metadata {
  const registry = getLiveCityRegistryEntry(city);
  const titleCity = registry?.name ?? beautifyCategory(city);

  return {
    title: `${titleCity} Now | City Pulse Right Now`,
    description: `Real-time ${titleCity} awareness layer for what is happening now, what is building tonight, and what matters around your anchor.`,
    alternates: {
      canonical: `/${city}/now`,
    },
  };
...
export async function LiveCityNowPage({
  city,
  searchParams,
}: {
  city: LiveCityKey;
  ...
  const defaultAnchor = bundle.anchors.anchors[0]?.slug ?? "";
  const requestedAnchor = typeof params.anchor === "string" && params.anchor ? params.anchor : defaultAnchor;
  const anchor = getLiveCityAnchor(city, requestedAnchor) ?? getLiveCityAnchor(city, defaultAnchor);

  if (!registry || !anchor) {
    throw new Error(`${city} live-city pack is missing required registry or anchor data.`);
  }

  const venuesBySlug = new Map(bundle.venues.venues.map((venue) => [venue.slug, venue]));
  const placesBySlug = new Map(bundle.places.places.map((place) => [place.slug, place]));
  ...
  const tonightSignals = bundle.signals.signals
    ...
    .sort((a, b) => {
      ...
      const scoreB = scoredAnchorSignals.find((entry) => entry.signal.id === b.id)?.score ?? 0;
      return scoreB - scoreA;
    });

  const topSignal =
    scoredAnchorSignals.find((entry) => entry.signal.status === "active" || entry.signal.impact_level === "high")?.signal ??
  ...
  const distinctAnchorSignals = scoredAnchorSignals
    .filter((entry) => entry.signal.id !== topSignal?.id && entry.score >= 0.45)
    .map((entry) => entry.signal)
    .slice(0, 3);
  const placeRail = Array.from(new Map([...anchorPlaces, ...bundle.places.places].map((place) => [place.slug, place])).values()).slice(0, 7);

  const rightNowFooter = [
    topSignal?.status ? titleize(topSignal.status) : null,
    topSignal?.affected_district_slugs?.[0] ? beautifyCategory(topSignal.affected_district_slugs[0]) : null,
    topSignal?.linked_venue_slug ? beautifyCategory(topSignal.linked_venue_slug) : null,
  ].filter(Boolean);
        ...
        <header className="overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(135deg,rgba(10,15,27,0.96),rgba(17,24,39,0.92))] p-6 shadow-[0_28px_80px_rgba(0,0,0,0.38)]">
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.26em] text-amber-200">
              <span>{registry.name} Now</span>
            </div>
            <div className="space-y-3">
              <h1 className="max-w-3xl text-4xl font-black tracking-tight sm:text-5xl">What matters right now and what is building tonight</h1>
              <p className="max-w-3xl text-base leading-7 text-zinc-300">
                Real-time city awareness with a point of view. See the main thing affecting {registry.name} now,
                what is building later today, and what matters around your anchor.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {registry.modes.map((mode) => (
                <span key={mode} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.18em] text-zinc-200">
            ...
            <div className="flex flex-wrap gap-2">
              {bundle.anchors.anchors.map((candidate) => {
                ...
                return (
                  <Link
                    key={candidate.slug}
                    href={`/${city}/now?anchor=${encodeURIComponent(candidate.slug)}`}
                    className={
                      isActive
          ...
          <section id="right-now" className="space-y-4">
            ...
            {topSignal ? (
              <article className="rounded-[2rem] border border-rose-300/20 bg-[linear-gradient(180deg,rgba(127,29,29,0.24),rgba(10,15,27,0.95))] p-6 shadow-[0_22px_60px_rgba(0,0,0,0.32)]">
                <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.22em]">
                  <span className="rounded-full border border-rose-300/30 bg-rose-500/10 px-2.5 py-1 text-rose-100">{titleize(topSignal.impact_level)}</span>
                  <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-zinc-300">
                    {topSignal.affected_district_slugs?.[0] ? beautifyCategory(topSignal.affected_district_slugs[0]) : "Citywide"}
                  </span>
                </div>
                <h2 className="mt-4 max-w-3xl text-3xl font-black tracking-tight text-white sm:text-4xl">{topSignal.title}</h2>
              ...
              <div className="rounded-[1.6rem] border border-white/10 bg-white/5 p-5 text-zinc-300">No ranked top signal is available yet.</div>
            )}
          </section>
          ...
          <section id="later-today" className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              {upcomingSignals.length > 0 ? (
                upcomingSignals.map((signal) => <SignalCard key={signal.id} signal={signal} timeZone={registry.timezone} />)
              ) : (
                <div className="rounded-[1.6rem] border border-white/10 bg-white/5 p-5 text-zinc-300 lg:col-span-3">
                  No scheduled tonight signals are in the {registry.name} seed pack yet.
                </div>
              )}
            </div>
          </section>
                    ...
                    <div className="mt-4 grid gap-3">
                      {anchorDistricts.map((district) => (
                        <div key={district.slug} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                          ...
                          <p className="mt-1 text-sm text-zinc-300">{district.vibe_tags.slice(0, 3).map(beautifyCategory).join(" • ")}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="rounded-[1.4rem] border border-white/10 bg-black/20 p-4">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {distinctAnchorSignals.map((signal) => (
                          <span key={signal.id} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-200">
                            {signal.title}
                          </span>
                        ))}
                      </div>
                      ...
                      <article key={place.slug} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {place.tags.slice(0, 2).map((tag) => (
                            <span key={`${place.slug}:${tag}`} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-zinc-200">
              ...
              {bundle.districts.districts.map((district) => (
                <article key={district.slug} className="rounded-[1.8rem] border border-white/10 bg-white/5 p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs text-zinc-300">{titleize(district.impact_profile)}</span>
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2">
