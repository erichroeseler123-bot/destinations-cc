    ...
    <main className="min-h-screen bg-[#050816] text-white">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10 sm:px-6 lg:px-8">
        ...
        <section className="rounded-[32px] border border-cyan-300/20 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.14),transparent_34%),linear-gradient(180deg,rgba(9,16,31,0.98),rgba(4,8,17,0.99))] p-7 sm:p-9">
          ...
          {district.vibe_tags?.length ? <div className="mt-5 flex flex-wrap gap-2">{district.vibe_tags.slice(0, 6).map((tag) => <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/70">{tag}</span>)}</div> : null}
          {streetViewHref ? <a href={streetViewHref} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex rounded-full border border-cyan-300/20 bg-cyan-300/10 px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-cyan-100">Look around in Street View →</a> : null}
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">Durable context</p><div className="mt-2 text-3xl font-black">{venues.length + places.length || (district.vibe_tags?.length || 0)}</div><p className="mt-2 text-sm text-white/50">Static anchors and district identity</p></div>
        </section>
        ...
        <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-6 sm:p-7">
          ...
          {(liveDistrict?.signals?.length || liveDistrict?.events?.length) ? (
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {(liveDistrict?.events || []).map((event) => <article key={event.id} className="rounded-[20px] border border-white/10 bg-black/20 p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-white/45">event {event.start ? `• ${new Date(event.start).toLocaleString()}` : ""}</p><h2 className="mt-2 text-lg font-bold">{event.name}</h2><p className="mt-2 text-sm text-white/55">{event.venue || event.category || "Current city event"}</p></article>)}
              {(liveDistrict?.signals || []).map((signal) => <article key={signal.id} className="rounded-[20px] border border-white/10 bg-black/20 p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-white/45">{signal.kind} • {signal.provider}</p><h2 className="mt-2 text-lg font-bold">{signal.title}</h2></article>)}
            </div>
          ) : signals.length ? (
