...
type LiveCityPayload = {
  ok?: boolean;
  freshness?: { signals?: { as_of?: string; stale_after?: string; ttl_seconds?: number } };
  bundle?: {
...
export default function CityWatchPanel({ citySlug, cityName, suppressRepoSignals = false, lat, lng, timezone }: {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const requests: Promise<Response>[] = [fetch(`/api/internal/live-city/${encodeURIComponent(citySlug)}`, { cache: "no-store", headers: { "x-dcc-surface": "city-watch" } })];
        if (typeof lat === "number" && typeof lng === "number") {
          const query = new URLSearchParams({ city: citySlug, lat: String(lat), lng: String(lng), timezone: timezone || "auto" });
          requests.push(fetch(`/api/public/city-live?${query.toString()}`, { cache: "no-store" }));
        }
        const responses = await Promise.all(requests);
        if (!responses[0].ok) throw new Error(String(responses[0].status));
        const next = (await responses[0].json()) as LiveCityPayload;
        const liveNext = responses[1]?.ok ? ((await responses[1].json()) as PublicLivePayload) : null;
        if (cancelled) return;
        setPayload(next);
        setPublicLive(liveNext);
  ...
  const districtNowBySlug = useMemo(() => new Map((publicLive?.districtNow || []).map((item) => [item.slug, item])), [publicLive]);
  ...
  }, [payload, freshSignals, districtNowBySlug]);
      ...
      <section className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(7,12,25,0.97),rgba(4,8,17,0.98))] p-6 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 text-sm leading-6 text-white/68">Durable district geography underneath; ephemeral live events and geolocated public signals on top. Citywide alerts stay citywide rather than being falsely pinned to one neighborhood.</p>
          </div>
          <span className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/64">
            {status === "loading" ? "checking" : publicLive?.districtNow?.length ? "district states live" : fresh ? "fresh public signals" : "district structure live"}
          </span>
        </div>
              ...
              <Link key={district.slug} href={`/${citySlug}/watch/${district.slug}`} className="group rounded-[22px] border border-white/10 bg-white/[0.045] p-4 transition hover:-translate-y-0.5 hover:border-cyan-300/25 hover:bg-white/[0.07]">
                ...
                {liveState ? (
                  <div className="mt-4 space-y-2">
                    {(liveState.events || []).slice(0, 2).map((event) => (
                      <div key={event.id} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                        <p className="text-sm font-semibold text-white/88">{event.name}</p>
                        <p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-cyan-200/65">{event.venue || "live event"}</p>
                      </div>
                    ))}
                    {(liveState.signals || []).slice(0, 2).map((signal) => (
                      <div key={signal.id} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                        ...
                        <p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-cyan-200/65">{signal.kind} • {signal.provider}</p>
                      </div>
                    ))}
                ...
                ) : !suppressRepoSignals && signals.length ? (
                  <div className="mt-4 space-y-2">{signals.map((signal) => <div key={signal.id} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2"><p className="text-sm font-semibold text-white/88">{signal.title}</p></div>)}</div>
                ) : <p className="mt-4 text-sm leading-5 text-white/42">Permanent district identity and Street View are available; no verified district-level live signal is mapped right now.</p>}
        ...
        <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-xs text-white/38">
          ...
          <span>Refreshes every minute</span><span>Dynamic request: no-store</span><span>Only geolocated signals are neighborhood-mapped</span>
        </div>
      </section>
