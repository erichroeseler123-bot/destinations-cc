...
export function generateStaticParams() {
  return listOverlayCitySlugsByType("accessibility").flatMap((city) =>
    listOverlayCategoriesByTypeAndCity("accessibility", city).map((category) => ({
...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  ...
  const overlay = getOverlayRegistryNodeByTypeAndCity("accessibility", city);
  const cityNode = getCityRegistryNode(city);
  if (!overlay || !cityNode) return {};
  return buildOverlayCategoryMetadata(cityNode, overlay, category, `/accessibility/${category}/${city}`);
}
