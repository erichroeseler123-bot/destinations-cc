...
export default function DccHydrationDemo({
  ...
  const links = Array.isArray(wellKnownPayload?.links) ? wellKnownPayload.links : [];
  ...
  const regionsClaim = claims.find((c: any) => c.predicate === "service_regions")?.value || [];
  const hubsClaim = claims.find((c: any) => c.predicate === "airport_hubs")?.value || [];
  const destinationsClaim = claims.find((c: any) => c.predicate === "destinations_served")?.value || [];
          ...
          <div className="rounded-2xl border border-white/10 bg-[#090e17] p-5">
            ...
            <div className="mt-4 border-t border-white/10 pt-3 text-xs space-y-2 text-white/70">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-[11px] text-emerald-300">{new Date(freshUntil).toLocaleTimeString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-white/40">State Freshness:</span>
                <span className="text-emerald-400 font-bold">VERIFIED FRESH</span>
              </div>
            </div>
          </div>
