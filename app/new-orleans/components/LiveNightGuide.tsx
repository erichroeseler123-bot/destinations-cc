  ...
  return (
    <main className="min-h-screen bg-[#151515] text-[#fdfbf7] px-6 py-14 md:py-20">
      <div className="mx-auto max-w-6xl">
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#d4af37] mb-4">{eyebrow}</p>
        <h1 className="font-[var(--font-accent)] text-4xl md:text-6xl font-bold leading-tight max-w-4xl">{title}</h1>
        <p className="mt-5 max-w-3xl text-lg text-[#cccccc] leading-relaxed">{intro}</p>

        <div className="mt-8 flex flex-wrap gap-3 text-sm">
          <Link href="/guides/whats-happening" className="border border-[#444] px-4 py-2 hover:border-[#d4af37]">What’s happening</Link>
          <Link href="/guides/tonight" className="border border-[#444] px-4 py-2 hover:border-[#d4af37]">Tonight</Link>
          <Link href="/guides/this-weekend" className="border border-[#444] px-4 py-2 hover:border-[#d4af37]">This weekend</Link>
        </div>
                  ...
                  <article key={event.id} className="overflow-hidden border border-[#2a2a2a] bg-[#1a1a1a] flex flex-col">
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="flex flex-wrap gap-2 mb-3">
                        <span className="text-[10px] uppercase tracking-widest font-bold text-[#d4af37] border border-[#d4af37]/40 px-2 py-1">{eventCategory(event)}</span>
                      </div>
                      ...
                      <div className="mt-6 pt-5 border-t border-[#2a2a2a] flex flex-col gap-3">
                        {event.url ? (
                          <a href={event.url} target="_blank" rel="noopener noreferrer" className="inline-flex justify-center bg-[#d4af37] text-[#151515] font-bold px-4 py-3 text-xs uppercase tracking-wider">Check tickets & details</a>
                        ) : null}
                        <Link href="/guides/where-to-eat#new-orleans-staples" className="inline-flex justify-center border border-[#444] text-[#fdfbf7] font-bold px-4 py-3 text-xs uppercase tracking-wider hover:border-[#d4af37]">Find dinner before it</Link>
                      </div>
                    </div>
                  </article>
                ))}
