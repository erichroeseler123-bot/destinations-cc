...
type GuideLink = { href: string; title: string; description: string };
type GuideSection = { eyebrow: string; title: string; intro: string; links: GuideLink[] };

const sections: GuideSection[] = [
  {
    eyebrow: "Right now", title: "You are already here", intro: "Use current timing first when the decision is for today, tonight, or this weekend.", links: [
  ...
  {
    eyebrow: "Time first", title: "How much time do you actually have?", intro: "A good New Orleans plan changes completely when you have four hours instead of a full day.", links: [
  ...
  {
    eyebrow: "Who is with you?", title: "Plan for the people, not an average tourist", intro: "Age mix, mobility, pace, and group type usually matter more than a generic top-ten list.", links: [
  ...
  {
    eyebrow: "Comfort & logistics", title: "Transportation, walking, weather, and mobility", intro: "These are often the constraints that determine whether an experience is actually a good fit.", links: [
  ...
  {
    eyebrow: "Choose the experience", title: "Compare the big New Orleans decisions", intro: "Use the page that already owns the decision instead of opening twenty tabs or creating duplicate search pages.", links: [
      { href: "/food-tours", title: "Food & cocktail tours", description: "Compare the currently bookable cocktail-focused walking experiences and what they actually include." },
      { href: "/ghost-tours", title: "Ghost & haunted-history tours", description: "Compare after-dark walking format, age fit, folklore, and practical route considerations." },
  ...
  {
    eyebrow: "Cruise visitors", title: "Fit New Orleans around your ship", intro: "Embarkation and disembarkation days need different planning from a normal hotel stay.", links: [
...
const quickStarts = [
  ...
  ["/booking-help", "Booking Help", "Get help understanding a booking path or what to confirm before checkout."],
] as const;

export default function GuidesPage() {
  return (
    <div className="min-h-screen bg-[var(--nola-bg-charcoal)] text-[var(--nola-ivory)]">
      <section className="border-b border-[var(--nola-border)] px-6 py-16 md:py-24"><div className="mx-auto max-w-6xl"><p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--nola-gold)]">New Orleans planning guides</p><h1 className="mt-4 max-w-4xl font-serif text-4xl leading-tight md:text-6xl">Start with the question that matches your trip</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--nola-text-muted)]">You do not need another giant list of attractions. Start with how much time you have, who is traveling, what the weather is doing, whether transportation matters, and what kind of experience sounds right.</p><div className="mt-9 flex flex-wrap gap-3"><Link href="/guides/things-to-do-in-new-orleans-today" className="bg-[var(--nola-gold)] px-5 py-3 text-sm font-bold uppercase tracking-[0.12em] text-black">What should we do today?</Link><Link href="/help-me-choose" className="border border-[var(--nola-gold)] px-5 py-3 text-sm font-bold uppercase tracking-[0.12em] text-[var(--nola-gold)]">Help Me Choose</Link></div></div></section>
      <section className="px-6 py-10"><div className="mx-auto grid max-w-6xl gap-3 sm:grid-cols-2 lg:grid-cols-4">{quickStarts.map(([href, title, description]) => <Link key={href} href={href} className="border border-[var(--nola-border)] bg-[var(--nola-surface-strong)] p-5 hover:border-[var(--nola-gold)]"><div className="font-serif text-xl text-[var(--nola-gold)]">{title}</div><p className="mt-2 text-sm leading-6 text-[var(--nola-text-muted)]">{description}</p></Link>)}</div></section>
      <DailyBriefSignup source="guides" />
      <div className="px-6 pb-20 pt-14"><div className="mx-auto max-w-6xl space-y-14">{sections.map((section) => <section key={section.title}><div className="max-w-3xl"><p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[var(--nola-gold)]">{section.eyebrow}</p><h2 className="mt-2 font-serif text-3xl md:text-4xl">{section.title}</h2><p className="mt-3 leading-7 text-[var(--nola-text-muted)]">{section.intro}</p></div><div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{section.links.map((link) => <Link key={link.href} href={link.href} className="group border border-[var(--nola-border)] bg-[var(--nola-surface-subtle)] p-5 transition hover:border-[var(--nola-gold)] hover:bg-[var(--nola-surface-strong)]"><h3 className="font-serif text-xl text-[var(--nola-ivory)] group-hover:text-[var(--nola-gold)]">{link.title}</h3><p className="mt-2 text-sm leading-6 text-[var(--nola-text-muted)]">{link.description}</p><span className="mt-4 inline-block text-xs font-bold uppercase tracking-[0.14em] text-[var(--nola-gold)]">Read guide →</span></Link>)}</div></section>)}<section className="border border-[var(--nola-gold)]/40 bg-[var(--nola-surface-strong)] p-7 md:p-9"><p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[var(--nola-gold)]">Still not sure?</p><h2 className="mt-2 font-serif text-3xl">Tell us what kind of day you want.</h2><p className="mt-3 max-w-2xl leading-7 text-[var(--nola-text-muted)]">The guide library is here when you want to research. If you would rather answer a few questions and get a shorter list, use Help Me Choose.</p><div className="mt-6 flex flex-wrap gap-3"><Link href="/help-me-choose" className="bg-[var(--nola-gold)] px-5 py-3 text-sm font-bold uppercase tracking-[0.12em] text-black">Start Help Me Choose</Link><Link href="/booking-help" className="border border-[var(--nola-border)] px-5 py-3 text-sm font-bold uppercase tracking-[0.12em] text-[var(--nola-ivory)]">Booking Help</Link></div></section></div></div>
    </div>
  );
}
