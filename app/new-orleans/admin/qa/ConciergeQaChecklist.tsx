...
const sections: ChecklistSection[] = [
  { id: "visual-brand", title: "Visual & Brand", items: [
  ...
  { id: "copy-positioning", title: "Copy & Positioning", items: [
  ...
  { id: "chooser", title: "Chooser / Recommendation Engine", items: [
  ...
  { id: "live-intelligence", title: "48-Hour Intelligence Layer", items: [
  ...
  { id: "performance", title: "Performance & Tech", items: [
    ...
    <div className="min-h-screen bg-[#09090a] text-[#f4ead3]">
      <header className="border-b border-[#342d20] bg-[#0d0d0e]">
        <div className="mx-auto w-[min(1180px,calc(100%-2rem))] py-10">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-6 text-[#bdb5a7]">The operating checklist for brand, recommendation quality, booking integrity, live intelligence, and production health. Checks are saved in this browser.</p>
          <div className="mt-6 flex flex-wrap items-center gap-4">
            <div className="h-2 w-64 overflow-hidden rounded-full bg-[#24211b]"><div className="h-full bg-[#c9a86a] transition-all" style={{ width: `${percent}%` }} /></div>
            <span className="text-sm font-semibold text-[#e8d8b4]">{completed} / {checklistIds.length} complete · {percent}%</span>
            <button type="button" onClick={() => { setChecked({}); try { window.localStorage.removeItem(STORAGE_KEY); } catch {} }} className="ml-auto border border-[#4a4130] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-[#c9a86a] hover:border-[#c9a86a]">Reset checks</button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto grid w-[min(1180px,calc(100%-2rem))] gap-8 py-10 lg:grid-cols-[1fr_1.05fr]">
              ...
              <div className="mt-4 space-y-3">
                {section.items.map((item) => (
                  <label key={item.id} className="flex cursor-pointer gap-3 border-t border-[#211f1a] pt-3 first:border-t-0 first:pt-0">
                    <input type="checkbox" checked={Boolean(checked[item.id])} onChange={(event) => setItem(item.id, event.target.checked)} className="mt-1 h-4 w-4 accent-[#c9a86a]" />
        ...
        <section className="h-fit border border-[#4a3d26] bg-[#0f0e0b] p-5 md:p-6 lg:sticky lg:top-6">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#c9a86a]">21-for-21 booking audit</p><h2 className="mt-2 font-serif text-3xl">Current experience inventory</h2></div>
          ...
          <div className="mt-6 space-y-2">
            {tours.map((tour, index) => {
              ...
              const mapping = tour.itemId ? "Item-level" : tour.flowId ? "Flow-level" : tour.variantCount > 0 ? "Variant-mapped" : "Needs mapping";
              const mappingClass = mapping === "Item-level" ? "border-[#40523b] text-[#9dbb91]" : mapping === "Flow-level" ? "border-[#66522f] text-[#d5b36d]" : mapping === "Variant-mapped" ? "border-[#40523b] text-[#9dbb91]" : "border-[#6a3434] text-[#d78b8b]";
              return (
                <label key={tour.id} className="grid cursor-pointer grid-cols-[auto_1fr_auto] gap-3 border border-[#252119] bg-[#12110e] p-3">
                  <input type="checkbox" checked={Boolean(checked[id])} onChange={(event) => setItem(id, event.target.checked)} className="mt-1 h-4 w-4 accent-[#c9a86a]" />
                  <div>
                    <div className="flex gap-2"><span className="text-[10px] font-bold text-[#776e60]">{String(index + 1).padStart(2, "0")}</span><p className={`text-sm font-semibold ${checked[id] ? "text-[#797268] line-through" : "text-[#eee3cc]"}`}>{tour.title}</p></div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.12em]">
                      <span className={`border px-2 py-1 ${mappingClass}`}>{mapping}</span>
                      {tour.variantCount > 0 && <span className="border border-[#34302a] px-2 py-1 text-[#aaa295]">{tour.variantCount} variants</span>}
                    </div>
                  </div>
                  <a href={`/tours/${tour.slug}`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="self-start text-[10px] font-bold uppercase tracking-[0.14em] text-[#c9a86a] hover:text-[#f0d79f]">Open ↗</a>
                </label>
              );
            })}
          </div>

          <div className="mt-5 border-t border-[#30291d] pt-4 text-xs leading-5 text-[#9d9587]"><strong className="text-[#d8bd84]">Audit rule:</strong> check a tour only after the detail page renders, its See Availability path reaches the intended FareHarbor destination, and every visible variant is valid. Flow-level products are allowed but should remain explicitly noted.</div>
        </section>
      </main>
    </div>
  );
