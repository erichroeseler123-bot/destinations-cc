...
import {
  buildAskDccPrompt,
  fallbackAskDccAnswer,
  getAskDccEvidence,
} from "@/lib/dcc/ask/service";
...
export async function POST(request: NextRequest) {
  ...
  if (!question) {
    return NextResponse.json({ error: "Ask DCC needs a question." }, { status: 400 });
  }
  ...
  if (apiKey && evidence.sources.length > 0) {
    try {
      ...
      const model = client.getGenerativeModel({
        model: process.env.DCC_AI_MODEL || "gemini-3.6-flash",
      });
      const prompt = buildAskDccPrompt({ question, history: messages, evidence });
  ...
  return NextResponse.json({
    ...
    sources: evidence.sources.map(({ answer: _answer, ...source }) => source),
