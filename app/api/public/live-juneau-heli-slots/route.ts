...
export async function GET(request: Request) {
  try {
    const market = "juneau-heli" as const;
    const shortname = getMarketShortname(market);

    if (!shortname) {
      return NextResponse.json({ error: "Missing FareHarbor Juneau shortname" }, { status: 500 });
    }
    ...
    const nestedSlots = await Promise.all(
      eligibleItems.map(async (item) => {
        const availabilities = await fetchAvailabilitiesForItem(item.pk || "", date, date);
    ...
    const message = error instanceof Error ? error.message : "Live Juneau helicopter slots unavailable";
