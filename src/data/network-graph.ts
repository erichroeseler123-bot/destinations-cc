...
const vibeBridgeNodes = [
  {
    id: "dcc:vibe:hub",
    url: "https://www.destinationcommandcenter.com/vibe-around",
    type: "commerce_research_bridge",
    label: "Vibe Around Decision Center",
    role: "catch uncertainty before driver selection",
  },
  ...VIBE_ISLANDS.map((island) => ({
    id: `dcc:vibe:${island.slug}`,
    url: `https://www.destinationcommandcenter.com/vibe-around/${island.slug}`,
    type: "island_decision_endpoint",
    label: `${island.label} Vibe Around Decision Center`,
    role: "resolve port-day uncertainty while preserving Vibe context",
  })),
];
...
const vibeBridgeEdges = [
  {
    from: "vibe",
    to: "dcc:vibe:hub",
    relation: "research_when_not_transaction_ready",
  },
  ...VIBE_ISLANDS.flatMap((island) => [
    {
      from: "vibe",
      to: `dcc:vibe:${island.slug}`,
      relation: "island_research_entrypoint",
    },
    {
      from: "dcc:vibe:hub",
      to: `dcc:vibe:${island.slug}`,
      relation: "narrows_to_island_context",
    },
    ...VIBE_GUIDE_SLUGS.map((slug) => ({
      from: `dcc:vibe:${island.slug}`,
      to: `dcc:guide:${slug}`,
      relation: "resolves_pre_booking_uncertainty",
    })),
  ]),
  ...VIBE_GUIDE_SLUGS.map((slug) => ({
    from: `dcc:guide:${slug}`,
    to: "vibe",
    relation: "contextual_return_after_research",
  })),
];
