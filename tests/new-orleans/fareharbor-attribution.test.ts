...
test("FareHarbor Lightframe attribution", async (t) => {
  ...
  await t.test("keeps all six raw fallback URLs byte-for-byte unchanged", () => {
    ...
    const actual = STOREFRONT_PRODUCTS.map((product) =>
  ...
  await t.test("wires homepage, catalog, chooser, guide, and detail sources", () => {
    ...
    assert.match(
      read("app/new-orleans/tours/OutpostConsole.tsx"),
      /attributionSource=\{FAREHARBOR_SOURCES\.tours\}/,
    );

    const productCard = read("app/new-orleans/components/ProductCard.tsx");
    assert.match(
      productCard,
